...
const checkOut = async (req, res) => {
  try {
    ...
    //categoryoffer
    const categoryOfferName = products.map((product, index) => {
      return product.productId ? product.productId.category : null;
    });
    ...
    if (categoryOfferName) {
      ...
      const calculatedTotals = categoryOfferData.map((item, index) => {
        const quantity = totalqty[index] || 0;
